    ...
    class MaxRectsBin extends Bin {
        ...
        place(rect) {
            ...
            if (node) {
                ...
                this.pruneFreeList();
                this.verticalExpand = this.options.logic === exports.PACKING_LOGIC.FILL_WIDTH ? false : this.width > this.height ? true : false;
                rect.x = node.x;
                rect.y = node.y;
        ...
        findNode(width, height, allowRotation) {
            ...
            for (let i in this.freeRects) {
                ...
                if (r.width >= width && r.height >= height) {
                    if (this.options.logic === exports.PACKING_LOGIC.MAX_AREA) {
                        // the rect with the lowest rest area wins
                        areaFit = r.width * r.height - width * height;
                    }
                    else if (this.options.logic === exports.PACKING_LOGIC.FILL_WIDTH) {
